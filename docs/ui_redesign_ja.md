...
## 15. サイドメニューの情報設計（Issue #29 / 2026-08）

### 15.1 何が問題だったか（計測結果）

| 症状 | 実測 |
|---|---|
| 階層が見えない | グループ見出しと項目が**同色**（`--crt-fg-dim`）・**1px 差**（12/13px）・両方 ALL CAPS。5 グループ + 17 項目 = **22 行がフラットに見える** |
| アイコンが無意味 | 21 スロットに **8 種類のみ**。`nav-ops` は 4 回、`clock` は QUARANTINE と AUTO GUARD の**別機能同士**で重複 |
| 用語が内部語 | `NPUB` / `IP ACL` はデータ型名。`DECK` と `DASHBOARD` の違いが不明 |
| 意図が分断 | **BAN が 3 グループに分散**（NPUB / IP ACL / QUICK BAN）。QUARANTINE と AUTO GUARD も別グループ |

### 15.2 新しい IA（17 → 12 項目）

グループは**実装都合ではなく運用者の意図**で切る。

| グループ | 項目 | パス |
|---|---|---|
| **みる** | 管制卓 / ライブ / ログ | `/` `/live` `/logs` |
| **とめる** | ブロック / 一時停止 / 自動ガード | `/block` `/quarantine` `/auto-guard` |
| **ルール** | 投稿ポリシー / kind 制限 / DSL ルール | `/policy` `/kind` `/dsl` |
| **つなぐ** | 上流リレー / リレー情報 | `/relays` `/nip11` |
| **設定** | システム | `/system` |

統合したもの:
- **ブロック** = 旧 NPUB + IP ACL（タブ切替）。集約 API で npub / IP を同じ形で扱えるようになったため
- **DSL ルール** = 旧 DSL + Quick BAN（タブ切替）。Quick BAN は GUI で組む簡易 DSL であり相互変換 API もある
- **システム** = 旧 System + Telemetry（同一ページの節）
- **管制卓** = 旧 Deck + Dashboard

旧 URL（`/access/*` `/filter/*` `/backend/*` `/operations/*` および平坦な旧 URL）はすべて redirect を残す。

### 15.3 視覚階層の規範

- **グループ見出し** = 9px / 字間 0.28em / `#7a6448`。「区切り」であって選択肢ではないので、項目より暗く小さく
- **項目** = 13.5px / 通常ケース / `--crt-fg`。ここが読ませたい情報
- **`sub` ラベル** = 10px mono / `--crt-fg-dim` で英語名や一言説明
- **アイコンは全項目で固有**。親グループとも重複させない（重複したアイコンは見分けの助けにならず、ノイズになる）
- 「とめる」グループには**未対応件数バッジ**を出す

### 15.4 横スクロール禁止（キャプチャ検証で検出）

再編時のスクリーンショット検証で、**モバイル幅で画面全体が横スクロールしていた**（390px に対し実測 483〜531px）。
グリッド／フレックスの子は既定で `min-content` 未満に縮まないため、長い要素が 1 つあるとシェルごと横に伸びる。

規範として以下を守る:
- シェルとカードに `min-width: 0`
- カードヘッダと Pill は狭い幅で **`flex-wrap: wrap`**
- 整形済みテキスト（JSON プレビュー等）は**その要素の中だけで** `overflow-x: auto`
- `web/scripts/screenshot-console.mjs` のキャプチャ時に 390 / 768px で `scrollWidth <= clientWidth` を確認する
