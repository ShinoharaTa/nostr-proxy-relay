...
hex(64 桁) に加えて bech32 を受け付け、内部で hex に正規化する:
`npub1` / `note1` / `nevent1` / `nprofile1` / `nostr:` URI プレフィックス。
**`nsec1`（秘密鍵）は明示的に拒否**し、値はエラーにもログにも出さない（誤貼り付け事故対策）。
`nevent` / `nprofile` の**リレーヒント**は、上流プールに接続済みのものだけ問い合わせ先へ自動追加する
（未接続ヒントは `unusable_relay_hints` として結果に注記）。

#### 反応の収集（refs / 反応マトリクス）

`refs` に event id を渡すと、その投稿への**反応**を集める。
1 つの REQ に `#e` / `#q` / `#E` の複数フィルタ（NIP-01 の OR）を載せ、
**1 回の取得でリプライ(1/1111)・リアクション(7)・リポスト(6/16)・zap(9735) が混在して返る**
（実リレー検証済み）。kind 分離はローカルで行い、UI はチップで切り替える。

- NIP-10: 実測でスレッド深部からの返信も root マーカーを持つため、`#e` 1 本でスレッドほぼ全体が拾える
- `until` でページング（「さらに読む」）。実リレーで動作検証済み
- refs 指定時は root イベント本体も取得し「何への反応か」を表示する

#### 構造タグの除外（誤提案の防止）

リプライは構造上 **root への `e` タグ**と **root 投稿者への `p` タグ**を全件共有する。
これらを共通タグ判定から除外しないと、リプライ爆撃の調査で
「タグ p=**被害者** をブロック」という最悪の誤提案になる（実リレー検証で確認）。
`refs` と root 投稿者 pubkey に一致する e/E/q/a/p/P タグは共通タグ集計から除外する。
