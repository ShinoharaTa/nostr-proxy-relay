...
export const consoleText = {
  ja: {
    ...
    investigate: {
      title: 'イベント調査',
      noStore: '保存しません',
      intro: 'note1 / nevent1 / npub1 / hex を貼ると、上流リレーへその場で問い合わせて出所とパターンを解析します。「対象投稿」に note を入れるとリプライ・リアクション・リポストを一括収集できます。取得内容は解析にのみ使い、保存しません（nsec は受け付けません）。',
      idsLabel: 'event id',
      authorsLabel: 'pubkey (hex)',
      run: '調査する',
      running: '問い合わせ中…',
      needInput: 'event id か pubkey を入力してください',
      refsLabel: '対象投稿（この投稿への反応 = リプライ等を集める）',
      rootTitle: '対象の投稿',
      chipReply: 'リプライ',
      chipReaction: 'リアクション',
      chipRepost: 'リポスト',
      loadMore: 'さらに読む',
      noMore: 'これ以上ありません',
      unusableHints: (urls: string) => `リレーヒント ${urls} は上流プールに未接続のため使用しませんでした`,
      verdictTitle: '判定',
  ...
  en: {
    ...
    investigate: {
      ...
      intro: 'Paste note1 / nevent1 / npub1 / hex and the proxy queries upstream relays right now. Put a note into the Target field to collect its replies, reactions and reposts at once. Fetched content is used for analysis only and never stored (nsec is rejected).',
      idsLabel: 'event id',
      authorsLabel: 'pubkey (hex)',
      run: 'INVESTIGATE',
      running: 'querying…',
      needInput: 'Enter at least one event id or pubkey',
      refsLabel: 'Target note (collect replies etc. to it)',
      rootTitle: 'Target note',
      chipReply: 'REPLY',
      chipReaction: 'REACTION',
      chipRepost: 'REPOST',
      loadMore: 'LOAD MORE',
      noMore: 'Nothing more to load',
      unusableHints: (urls: string) => `Relay hints ${urls} were skipped (not in the upstream pool)`,
